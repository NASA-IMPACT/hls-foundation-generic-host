...
BASE_TILE_URL = "{BASE_URL}/mosaic/tiles/{searchid}/WebMercatorQuad/{z}/{x}/{y}@2x.tif"
...
class Downloader:
    ...
    def download_tile(self, x_index, y_index, filename):
        ...
        if response.status_code == 200:
            ...
            with rasterio.open(filename.replace('.tif', '_scaled.tif'), 'w', **profile) as scaled:
                for band in range(profile['count']):
                    index = band + 1
                    data = raster_file.read(index).astype('float32') * 0.0001
                    data = np.clip(data, 0, data.max())
                    scaled.write(data, index)
            with rasterio.open(filename, 'w', **profile) as unscaled:
                for band in range(profile['count']):
                    index = band + 1
                    data = raster_file.read(index).astype('float32')
                    unscaled.write(data, index)
